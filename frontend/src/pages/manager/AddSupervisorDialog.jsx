...
export default function AddSupervisorDialog({ open, onClose }) {
  const { enqueueSnackbar } = useSnackbar()
  const qc = useQueryClient()
  const { register, handleSubmit, reset, formState: { isSubmitting, errors } } = useForm()

  const mutation = useMutation({
    mutationFn: (data) => createUser({ ...data, role: 'supervisor' }),
    onSuccess: () => {
    ...
    onError: (err) => {
      const msg = err.response?.data?.email?.[0] || err.response?.data?.detail || 'Failed to create supervisor.'
