  ...
  return (
    <AppLayout title="Manager Dashboard">
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            {user?.department_name || 'Your department'} training overview
          </Typography>
        </Box>
        <Button variant="contained" startIcon={<AssignmentIcon />} onClick={() => navigate('/manager/assign')}>
          Assign Training
        </Button>
      </Box>
      ...
      <Grid container spacing={3} mb={4}>
        <Grid item xs={6} md={3}>
          <KpiCard title="Dept Assignments" value={summary?.total_assignments ?? '—'} icon={<AssignmentIcon />} color="primary" loading={loadingSummary} />
        </Grid>
        <Grid item xs={6} md={3}>
        ...
        <Grid item xs={6} md={3}>
          <KpiCard title="Supervisors" value={supervisors.length} icon={<ManageAccountsIcon />} color="info" />
        </Grid>
      </Grid>
          ...
          <Card>
            <CardContent>
              <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                <Typography variant="h6" fontWeight={600}>Supervisors</Typography>
                <Button size="small" onClick={() => navigate('/manager/supervisors')}>View All</Button>
              </Box>
              <Table size="small">
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell>Team Completion</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {supervisors.slice(0, 8).map((s) => (
                    <TableRow key={s.id} hover sx={{ cursor: 'pointer' }} onClick={() => navigate(`/manager/supervisors`)}>
                      <TableCell>
                      ...
                      <TableCell>
                        <Box display="flex" alignItems="center" gap={1}>
                          <LinearProgress
                            variant="determinate" value={s.completion_rate}
                            sx={{ flex: 1, height: 6, borderRadius: 3 }}
                            color={s.completion_rate >= 80 ? 'success' : s.completion_rate >= 50 ? 'warning' : 'error'}
                          />
                          <Typography variant="caption" fontWeight={600}>{s.completion_rate}%</Typography>
                        </Box>
                      </TableCell>
                    </TableRow>
                  ))}
