...
const STEPS = ['Select Trainings', 'Select Employees', 'Due Date & Notes', 'Confirm']

export default function AssignTrainingDialog({ open, onClose, preEmployee = null }) {
  ...
  useEffect(() => {
    if (open && preEmployee) setSelectedEmployees([preEmployee])
  }, [open, preEmployee])
      ...
      <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
        ...
        <DialogContent sx={{ textAlign: 'center', py: 4 }}>
          <Typography variant="h6" color="success.main" fontWeight={700} mb={1}>
            {result.created} assignment(s) created!
          </Typography>
          {result.skipped > 0 && (
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => { setResult(null); setStep(0); setSelectedTrainings([]); setSelectedEmployees([]) }}>
            Assign More
          </Button>
    ...
    <>
      <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
        ...
        <DialogContent dividers sx={{ minHeight: 460 }}>
                    ...
                    <ListItem key={t.id} disablePadding divider>
                      <ListItemButton onClick={() => toggleTraining(t)} dense>
                        <ListItemIcon sx={{ minWidth: 36 }}>
                          <Checkbox checked={!!selectedTrainings.find((x) => x.id === t.id)} size="small" edge="start" />
                        </ListItemIcon>
                        <ListItemText
              ...
              {selectedTrainings.length > 0 && (
                <Box mt={1.5} display="flex" gap={0.5} flexWrap="wrap">
                  ...
                  {selectedTrainings.map((t) => (
                    <Chip key={t.id} label={t.title} size="small" color="primary" variant="outlined" onDelete={() => toggleTraining(t)} />
                  ))}
                </Box>
              )}
            </Box>
          ...
          {step === 1 && (
            <Box>
              <Typography variant="body2" color="text.secondary" mb={1}>
                Select employee(s) to assign the training to:
              </Typography>
              {employees.length === 0 ? (
                <Box py={4} textAlign="center">
                  <Typography color="text.secondary" variant="body2">
                    No employees in your team yet. Add employees first.
                  </Typography>
                </Box>
              ) : (
                  ...
                  {employees.map((e) => (
                    <ListItem key={e.id} disablePadding divider>
                      <ListItemButton onClick={() => toggleEmployee(e)} dense>
                        <ListItemIcon sx={{ minWidth: 36 }}>
                          <Checkbox checked={!!selectedEmployees.find((x) => x.id === e.id)} size="small" edge="start" />
                        </ListItemIcon>
                        <ListItemText
              ...
              {selectedEmployees.length > 0 && (
                <Box mt={1.5} display="flex" gap={0.5} flexWrap="wrap">
                  ...
                  {selectedEmployees.map((e) => (
                    <Chip key={e.id} label={e.full_name} size="small" color="secondary" variant="outlined" onDelete={() => toggleEmployee(e)} />
                  ))}
                </Box>
              )}
            </Box>
          ...
          {step === 2 && (
            <Box display="flex" flexDirection="column" gap={3} maxWidth={420} pt={1}>
          ...
          {step === 3 && (
            <Box display="flex" flexDirection="column" gap={2.5}>
              ...
              <Alert severity="info" sx={{ mt: 1 }}>
                ...
                Employees who already have an active assignment for a training will be skipped.
              </Alert>
            </Box>
          )}
        </DialogContent>
